...
class GuildProfileView(discord.ui.View):
    def __init__(self, transactions_table, g_profile_embed) -> None:
    ...
    async def guild_balance(
        ...
        local_table = self.transactions_table
        await interaction.response.defer()
        if self.gb_pressed == 0:
            ...
            embed_guild.add_field(
                ...
                value=f"```{local_table.select('ITEM', 'QUANTIDADE').group_by('ITEM').sum().sort('QUANTIDADE', descending=True)}```",
                inline=True,
            )
            self.gb_pressed = 1
            self.update_buttons()
