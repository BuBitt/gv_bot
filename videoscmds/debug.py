...
async def debug(
    ctx,
    item: typing.Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel],
):
    logger.info(item.__repr__)
    embed = discord.Embed(title=f"{item.name} Info")
    embed.add_field(name="ID", value=item.id, inline=False)
    embed.add_field(name="Type", value=item.type, inline=True)
    embed.add_field(
        name="Created at", value=discord.utils.format_dt(item.created_at), inline=False
    )

    if item.category:
        embed.add_field(
            name="Category",
            value=f"{item.category.name} (ID: {item.category.id})",
            inline=False,
        )
    ...
    if isinstance(item, discord.VoiceChannel):
        ...
        if item.members:
            ...
            embed.add_field(
                name="Active Members Info", value=members_list[:-2], inline=False
            )

    if isinstance(item, discord.TextChannel):
        embed.add_field(
            name="News?", value="Yes" if item.is_news() else "No", inline=True
        )
        embed.add_field(
            name="NSFW?", value="Yes" if item.is_nsfw() else "No", inline=True
        )
