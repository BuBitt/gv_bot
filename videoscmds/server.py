...
async def server(ctx):
    ...
    embed = discord.Embed(
        title=f"{ctx.guild.name} - Server Info ", description="Your server information"
    )
    ...
    embed.add_field(
        name="Created at",
        value=discord.utils.format_dt(ctx.guild.created_at),
        inline=False,
    )
    embed.add_field(
        name="Server Description", value=ctx.guild.description, inline=False
    )

    embed.add_field(name="Owner Name", value=ctx.guild.owner.nick, inline=False)
    embed.add_field(
        name="Owner Account Created",
        value=discord.utils.format_dt(ctx.guild.owner.created_at),
        inline=False,
    )
    ...
    embed.add_field(
        ...
        value=f"%.2f Mb" % float(ctx.guild.filesize_limit / 1024 / 1024),
        inline=False,
    )
    embed.add_field(
        name="Bitrate Limit",
        value=f"%.2f kbit" % float(ctx.guild.bitrate_limit / 1000),
        inline=False,
    )

    embed.add_field(name="Tier", value=ctx.guild.premium_tier, inline=True)
    embed.add_field(
        name="Sub Count", value=ctx.guild.premium_subscription_count, inline=True
    )
    ...
    embed.add_field(
        name="Is large?", value="Yes" if ctx.guild.large else "No", inline=False
    )
    ...
    if ctx.guild.premium_subscriber_role:
        embed.add_field(
            ...
            value=ctx.guild.premium_subscriber_role.name,
            inline=True,
        )
